...
const inbody = axios.create({
  ...
  headers: { 'Account': INBODY_ACCOUNT, 'API-KEY': INBODY_API_KEY, 'Content-Type': 'application/json' },
  timeout: 10000,
})
...
app.post('/webhook', async (req, res) => {
  ...
  const [result] = await db.execute(
    `INSERT INTO webhook_events (user_id, user_token, equip, equip_serial, test_at, account, is_temp, fetch_status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [p.UserID || '', p.TelHP || null, p.Equip || null, p.EquipSerial || null,
     p.TestDatetimes || '', p.Account || null, isTemp ? 1 : 0,
     isTemp ? 'skipped_temp' : (INBODY_ACCOUNT && INBODY_API_KEY ? 'pending' : 'skipped_no_config')],
  )
  const eventId = result.insertId

  console.log(`[Webhook] UserID=${p.UserID} Equip=${p.Equip} Time=${p.TestDatetimes}${isTemp ? ' (temp)' : ''}`)
  res.status(200).json({ received: true })

  if (!isTemp && INBODY_ACCOUNT && INBODY_API_KEY && p.UserID && p.TestDatetimes) {
...
async function fetchAndStore(eventId, userId, testAt, db) {
  try {
    const res = await inbody.post('/api/v1/InBody/GetInBodyDataByID', {
      USER_ID: userId, DATETIMES: testAt,
    })
    await db.execute(
...
app.get('/api/stats', async (_req, res) => {
  ...
  res.json({ todayMeasurements: today, totalMembers: total, weekMeasurements: week,
...
app.get('/api/members/:userId/inbody', async (req, res) => {
  if (!INBODY_ACCOUNT || !INBODY_API_KEY) return res.status(503).json({ error: 'API not configured' })
  try {
    // 1. Get datetime list
    const dtRes = await inbody.post('/api/v1/InBody/GetDatetimesByID', { USER_ID: req.params.userId })
    const datetimes = Array.isArray(dtRes.data) ? dtRes.data : (dtRes.data?.datetimes ?? [])
    ...
    for (const dt of toFetch) {
      try {
        const r = await inbody.post('/api/v1/InBody/GetInBodyDataByID', { USER_ID: req.params.userId, DATETIMES: dt })
        results.push({ datetimes: dt, ...r.data })
      } catch { /* skip */ }
    }
    res.json(results)
  } catch (err) {
    res.status(err.response?.status || 500).json({ error: err.response?.data?.errorCode || err.message })
  }
})
...
app.post('/api/members/:userId/notes', async (req, res) => {
  ...
  res.json({ id: result.insertId, user_id: req.params.userId, note: note.trim(), created_at: new Date() })
})

app.delete('/api/members/:userId/notes/:id', async (req, res) => {
  const db = await getPool()
  await db.execute(`DELETE FROM member_notes WHERE id = ? AND user_id = ?`, [req.params.id, req.params.userId])
...
app.post('/api/test-webhook', async (req, res) => {
  ...
  req.body = {
    EquipSerial: 'CC71700163', TelHP: '01012344733',
    UserID: req.body?.userID || 'testuser001',
    TestDatetimes: req.body?.testDatetimes || ts,
    Account: INBODY_ACCOUNT || 'demo', Equip: 'InBody770',
    Type: 'InBody', IsTempData: 'false',
  }
  // Reuse webhook handler
  const mockRes = { status: () => mockRes, json: () => {} }
  app._router.handle({ ...req, url: '/webhook', path: '/webhook' }, mockRes, () => {})
  ...
  const [result] = await db.execute(
    `INSERT INTO webhook_events (user_id, user_token, equip, equip_serial, test_at, account, is_temp, fetch_status)
     VALUES (?, ?, ?, ?, ?, ?, 0, ?)`,
    [p.UserID, p.TelHP, p.Equip, p.EquipSerial, p.TestDatetimes, p.Account,
     INBODY_ACCOUNT && INBODY_API_KEY ? 'pending' : 'skipped_no_config'],
  )
  const eventId = result.insertId
  if (INBODY_ACCOUNT && INBODY_API_KEY) fetchAndStore(eventId, p.UserID, p.TestDatetimes, db)
...
async function start() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`  API ready : ${INBODY_ACCOUNT && INBODY_API_KEY ? 'Yes' : 'No (set INBODY_ACCOUNT + INBODY_API_KEY)'}`)
      console.log('────────────────────────────────────────────')
    })
  } catch (err) {
